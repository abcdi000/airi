...
flowchart TD
  ...
  B --> C["既有记忆、情绪、关系和防御状态"]
  C --> D["意识模型 / Planner"]
  D --> E["既有 Tool Mesh、MCP 和插件工具"]
  E --> F["结构化 LumiReplyIntent"]
  F --> G["规则 + 可选向量召回"]
  G --> H["可选 Expression Selector，最多 0-3 条"]
  H --> I["无工具 Replyer"]
  I --> J["Validator"]
  J -->|失败一次| I
  J -->|再次失败| K["意图保持的安全短句"]
  J -->|通过| L["1-3 条可见消息"]
  L --> M["决策日志与下一轮反馈"]
  M --> N["熟悉度、ownership、affinity、反馈和衰减"]
```
...
- 桌面离线模式目前使用词法、场景、亲和度和反馈召回；服务器模式可复用已有向量进程。后续若桌面 IPC 暴露通用 embedding 批处理，可接入同一可选 `queryEmbedding` 字段。
