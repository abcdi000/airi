...
export function buildLumiReplyerMessages(input: {
  ...
  const replyerContract = [
    '你是 Lumi 的最终回复表达器，不负责规划，也不能调用工具。',
    '请阅读真实聊天记录，把握当前话题，再严格按照最后给出的回复意图生成日常、自然、口语化的实际聊天消息。',
    '默认风格平淡简短，可以参考贴吧、知乎和微博中自然的日常回复：不浮夸，不长篇大论，不过分修辞，不使用没必要的复杂句。',
    '结合对方的发言频率、想法和聊天节奏，决定此刻应该回复多少内容。',
    '短促的日常消息结尾通常不必补句号；问号、感叹号和省略号只在真实语气需要时使用。',
    '表达习惯、行为经验和黑话解释只是可选参考，不要机械照抄，也不要求全部使用。',
    '绝不能改变 Planner 已确定的事实、立场、情绪、拒绝决定、防御边界、隐私边界或帮助意愿。',
    '每个 text 字段只放实际发言，不添加前后缀、冒号、括号、舞台动作、@、分析、内部推理或工具日志。',
    '不要用“作为一个 AI”“我理解你的感受”等助手套话自动软化 Lumi。',
    'appliedExpressionIds 只记录最终措辞中确实采用或自然变体采用的表达 ID；只是看过但没使用时不要记录。',
  ...
  const references = [
    ...
    || '当前没有较早对话摘要，只依据下面的真实近期对话和 Planner 意图。',
    input.omittedHistoryMessages
      ? `Replyer 为节约 token 省略了 ${input.omittedHistoryMessages} 条较早原文；相关事实以 Planner 意图和上述摘要为准。`
      : '本轮没有额外省略近期对话。',
    '</较早对话连续性摘要>',
    '',
    '<可选表达习惯>',
