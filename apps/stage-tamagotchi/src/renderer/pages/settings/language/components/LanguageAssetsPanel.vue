...
const visibleAssetCount = shallowRef(40)
const assetBatchSize = 40
const assetViewport = useTemplateRef<HTMLDivElement>('assetViewport')
...
const activeAssetCount = computed(() => ({
  expressions: filteredExpressions.value.length,
  jargon: snapshot.value.jargon.length,
  behaviors: snapshot.value.behaviors.length,
  feedback: feedbackDecisions.value.length,
})[activeTab.value])
const visibleExpressions = computed(() => filteredExpressions.value.slice(0, visibleAssetCount.value))
const visibleJargon = computed(() => snapshot.value.jargon.slice(0, visibleAssetCount.value))
const visibleBehaviors = computed(() => snapshot.value.behaviors.slice(0, visibleAssetCount.value))
const visibleFeedbackDecisions = computed(() => feedbackDecisions.value.slice(0, visibleAssetCount.value))
const renderedAssetCount = computed(() => Math.min(visibleAssetCount.value, activeAssetCount.value))
const hasMoreAssets = computed(() => renderedAssetCount.value < activeAssetCount.value)

function loadMoreAssets(event: Event) {
  const target = event.currentTarget as HTMLElement
  const reachedEnd = target.scrollTop + target.clientHeight >= target.scrollHeight - 160
  if (reachedEnd && hasMoreAssets.value)
    visibleAssetCount.value += assetBatchSize
}

watch([activeTab, search, statusFilter], async () => {
  visibleAssetCount.value = assetBatchSize
  await nextTick()
  assetViewport.value?.scrollTo({ top: 0 })
})
    ...
    >
      <Transition name="language-panel" mode="out-in">
        <div v-if="activeTab === 'expressions'" key="expressions" :class="['flex flex-col gap-4']">
          ...
          <article
            v-for="expression in visibleExpressions"
            :key="expression.id"
            v-motion
        ...
        <div v-else-if="activeTab === 'jargon'" key="jargon" :class="['flex flex-col']">
          ...
          <article v-for="jargon in visibleJargon" :key="jargon.id" :class="['border-b border-neutral-200 py-4 dark:border-neutral-800']">
        ...
        <div v-else-if="activeTab === 'behaviors'" key="behaviors" :class="['flex flex-col']">
          ...
          <article v-for="behavior in visibleBehaviors" :key="behavior.id" :class="['border-b border-neutral-200 py-4 dark:border-neutral-800']">
        ...
        <div v-else key="feedback" :class="['flex flex-col']">
          ...
          <article v-for="decision in visibleFeedbackDecisions" :key="decision.id" :class="['border-b border-neutral-200 py-4 dark:border-neutral-800']">
      ...
      <div
        v-if="activeAssetCount"
        :class="[
          'border-t border-neutral-200 py-3',
          'text-center text-xs tabular-nums text-neutral-500',
          'dark:border-neutral-800',
        ]"
      >
        {{ renderedAssetCount }} / {{ activeAssetCount }}
      </div>
    </div>
  </section>
</template>
