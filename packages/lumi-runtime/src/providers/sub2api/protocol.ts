...
/** One model exposed by either an OpenAI list or a Codex manifest. */
export interface Sub2ApiModelInfo {
  id: string
  displayName?: string
  ownedBy?: string
  source: 'openai-list' | 'codex-manifest'
}

export type Sub2ApiInputContentPart
...
export type Sub2ApiInputItem
  = | {
    type: 'message'
    role: 'user' | 'assistant'
    content: string | readonly Sub2ApiInputContentPart[]
  }
  | {
    type: 'function_call'
    call_id: string
...
/** Parses both OpenAI `data[]` and Codex `models[]` model catalogues. */
export function parseSub2ApiModelList(payload: unknown): Sub2ApiModelInfo[] {
  const root = record(payload)
  if (!root || (!Array.isArray(root.data) && !Array.isArray(root.models))) {
    throw new Sub2ApiProviderError('invalid_response', 'Sub2API returned an invalid model list')
  }

  const models = [
    ...parseModelItems(array(root.data), 'openai-list'),
    ...parseModelItems(array(root.models), 'codex-manifest'),
  ]
  const unique = new Map<string, Sub2ApiModelInfo>()
...
export async function parseSub2ApiResponsesStream(
  ...
  try {
    for await (const payload of parseSseData(stream)) {
      ...
      applyResponsesEvent(event, state, options)
    }
  }
  catch (error) {
    if (error instanceof Sub2ApiProviderError) {
...
function applyResponsesEvent(
  ...
  callbacks: Pick<ParseStreamOptions, 'onTextDelta' | 'onReasoningDelta' | 'onToolCallDelta'>,
): void {
  ...
  if (type === 'response.function_call_arguments.delta') {
    ...
    callbacks.onToolCallDelta?.({
      callId: current.id,
      name: current.name,
      argumentsDelta: text(event.delta),
    })
    return
  }
  ...
  if (!state.text && normalized.text) {
    ...
    callbacks.onTextDelta?.(normalized.text)
  }
...
async function responseError(
  ...
  const kind = classifyHttpError(response.status, code, message)
...
/** Whether Auto mode may delegate an unsupported Responses endpoint to Chat Completions. */
export function canFallbackFromSub2ApiResponses(error: unknown): boolean {
...
function canFallbackFromResponses(error: unknown): boolean {
  return canFallbackFromSub2ApiResponses(error)
}
...
function parseModelItems(
  ...
  source: Sub2ApiModelInfo['source'],
): Sub2ApiModelInfo[] {
